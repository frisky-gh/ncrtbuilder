...
package NCRTBuild::PluginConfigDistributor::HostServiceForAgentless;
...
sub run ($) {
	...
	foreach my $e ( @$host_service_list ){
		...
		foreach my $g ( @$generator_list ){
			...
			$workdir->writeToAllHosts( $measurementtype, "$filename.$host.$service", @content );
		}
	}
}
